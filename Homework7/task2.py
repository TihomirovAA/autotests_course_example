...
class PersonInfo:
    ...
    def new_salary(self):
        ...
        for dep in self.departments:
            for letter in dep:
                if letter not in dep_counts:
                    dep_counts[letter] = 1
                else:
                    dep_counts[letter] += 1
        top_3 = sorted(dep_counts.items(), key=lambda x: x[1], reverse=True)[:3]
        sum_top_3 = sum([count for letter, count in top_3])
        return 1337 * self.age * sum_top_3
