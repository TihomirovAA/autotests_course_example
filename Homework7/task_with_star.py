...
class RomanNums:
    ...
    def from_roman(self):
        ...
        for i in range(len(self.roman_numeral)):
            if i > 0 and roman_dict[self.roman_numeral[i]] > roman_dict[self.roman_numeral[i - 1]]:
                arabic_numeral += roman_dict[self.roman_numeral[i]] - 2 * roman_dict[self.roman_numeral[i - 1]]
            else:
                arabic_numeral += roman_dict[self.roman_numeral[i]]
        return arabic_numeral

    def is_palindrome(self):
        number = self.from_roman()
